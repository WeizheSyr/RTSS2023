...
noauth_x_up = []
for i in range(len(x_hat_arr) - 1):
    fixed_x_low.append(x_hat_arr[i] + sys.noauth_theta[i][0][0])
    fixed_x_up.append(x_hat_arr[i] + sys.noauth_theta[i][0][1])
...
plt.plot(noauth_x_low, c='black', linestyle=':', label='noauth_x_low')
plt.plot(noauth_x_up, c='black', linestyle=':', label='noauth_x_up')

plt.subplot(4, 1, 2)
plt.plot(reach, c='red', linestyle=':', label='klevels')
plt.plot(fixed_reach, c='blue', linestyle=':', label='fixed_klevels')
plt.plot(noauth_reach, c='black', linestyle=':', label='noauth_klevels')
