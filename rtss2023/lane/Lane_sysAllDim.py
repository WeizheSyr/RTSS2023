...
class SystemALLDim:
    def __init__(self, detector, exp, attack=None, attack_duration=50):
        ...
        # authentication
        self.auth = Authenticate(exp.model, 1)    # 1 for lane
        ...
        # recovery-ability
        self.pz = Zonotope.from_box(np.ones(4) * -0.002, np.ones(4) * 0.002)    # process noise
        self.uz = Zonotope.from_box(np.ones(1) * -0.50, np.ones(1) * 0.50)
        self.target_low = np.array([-0.15, -0.25, -0.15, -0.25])
        self.target_up = np.array([0.15, 0.25, 0.15, 0.25])
        ...
        while True:
            ...
            if self.authTimestep == self.auth.timestep:
                ...
                self.time_auths.append(end_time - start_time)
            ...
            if len(self.authT) != 0 and self.authT[-1] != self.i:
                ...
                self.time_errors.append(end_time - start_time)
                # combine bound
                ...
                end_time = time.time()
                self.time_recovers.append(end_time - start_time)
                ...
                while(True):
                    if self.klevels[-1] - self.klevel < 0 or self.klevels[-1] - self.klevel > 3:
                        ...
                        end_time = time.time()
                        self.time_adjs.append(end_time - start_time)
