    ...
    def boundByDetector(self, t):
        ...
        for i in range(len(pOrN)):
            ...
            if pOrN[i] > 0:
                ...
                theta1[i][1] = (self.detector.tao - rsum + self.A @ (self.y_hat[t - 1] - self.y_tilda[t - 1]) + 0.0001)[i] + A_theta_up
            else:
                theta1[i][0] = (-self.detector.tao + rsum - self.A @ self.y_tilda[t - 1] + self.A @ self.y_hat[t - 1] - 0.0001)[i] + A_theta_lo
