...
class Simulator:
    ...
    def data_init(self):
        ...
        self.predict = np.empty((self.max_index + 2, self.n), dtype=float)
    ...
    def noise_init(self, noise):
        ...
        if 'process' in noise:
            if noise['process']['type'] == 'white':
                ...
                self.p_noise_dist_1 = GaussianDistribution.from_standard(miu, C)
                self.p_noise_1 = self.p_noise_dist_1.random(self.max_index + 2).T
